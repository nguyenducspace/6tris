...
function showHelp() {
	...
	$("#inst_main_body").html("<div id = 'instructions_head'>LUẬT CHƠI</div><p>Cách chơi giống trò Teris (Xếp Gạch) và nhớ đừng để mấy block vượt qua các cạnh xám của lục giác nha!</p><p>" + (settings.platform != 'mobile' ? 'Sử dụng phím mũi LEFT hoặc RIGHT' : 'Tap bên TRÁI hoặc bên PHẢI của màn hình') + " để xoay khối lục giác ở giữa!" + (settings.platform != 'mobile' ? ' Sử dụng phím DOWN để tăng tốc độ rơi của block': '') + " </p><p>Clear blocks bằng cách làm cho nhiều hơn 3 block cùng màu xếp gần nhau</p><p>Time left before your combo streak disappears is indicated by <span style='color:#f1c40f;'>the</span> <span style='color:#e74c3c'>colored</span> <span style='color:#3498db'>lines</span> <span style='color:#2ecc71'>on</span> the outer hexagon</p> <hr> <p id = 'afterhr'></p> By <a href='http://facebook.com/nguyenduc.space' target='_blank'>Nguyễn Đức</a>);
	if (gameState == 1) {
		pause();
	}
